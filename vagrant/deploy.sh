...
export CONCOURSE_URL=http://localhost:8081
...
if ! ( [ -a .vagrant/tunnel-ctrl-socket ] && \
  ...
  vagrant ssh -- -L 8081:127.0.0.1:8080 -fN \
    -M -S .vagrant/tunnel-ctrl-socket -o "ExitOnForwardFailure yes"
fi
if ! curl -f -qs http://localhost:8080/login -o /dev/null; then
  echo "Failed creating SSH tunnel to remote concourse: 'vagrant ssh -- -L 8081:127.0.0.1:8080 -N'"
fi
